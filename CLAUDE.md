...
- [`.claude/rules/theme-performance.md`](.claude/rules/theme-performance.md) — what "a theme is a layout" costs: why HTML is `private` and must never go back to `no-store` (it disables bfcache), why prefetch is `hover` not `viewport`, why the font `<link>` and `theme-color` are per-theme, the one-`h1`-per-variant + shared-copy content-parity rule, and why a second `astro:page-load` ScrollTrigger silently dies. Read before touching caching headers, `<head>`, prefetch config, or adding a theme/variant.
- [`.claude/rules/loading-states.md`](.claude/rules/loading-states.md) — the three loading tiers (cold-document boot overlay / navigation progress bar / async skeleton), why the boot loader must never run on client-side navigation, and why an animated overlay's failsafe can't itself depend on `requestAnimationFrame`. Read before touching any loading UI.
